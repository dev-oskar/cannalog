...
export async function handleNhostMiddleware(
  ...
  const nhost = createServerClient({
    ...
    storage: {
      ...
      get: (): Session | null => {
        ...
        console.log(`[NHOST-SERVER] Getting session from cookie: ${!!raw}`);
        if (!raw) {
          return null;
        }
        try {
      ...
      set: (value: Session) => {
        console.log(
          `[NHOST-SERVER] Setting session cookie for user: ${value.user?.id}`
        );
        cookies.set(key, JSON.stringify(value), {
          path: "/",
          httpOnly: false, // if set to true we can't access it in the client
      ...
      remove: () => {
        console.log(`[NHOST-SERVER] Removing session cookie`);
        cookies.delete(key);
      },
    },
  });
  ...
  console.log(
    `[NHOST-SERVER] Session after refresh: ${!!refreshedSession}, User: ${!!refreshedSession?.user}`
  );
